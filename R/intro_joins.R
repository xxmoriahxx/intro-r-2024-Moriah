...
# use the data_detectors_meta to join with the stations metadata - find common attribute
# stationid is common variable
# I want the stations metadata for the detectors that we have data for
# add the info from the stations metadata to the detector metadata so that it has all the info about the station

data_stations <- data_detectors_meta |>
  select (detector_id, stationid) |>
  left_join (stations, by = "stationid") #don't need c because variable has same name both files
head(data_stations)
# to the table about the detectors, add the information about the station (a station has multiple detectors)
